...
```
 dfarm CLI / REST clients          dashboard (browser)
            │                              │ WS/SSE
            ▼                              ▼
        apps/web  ── Next.js: API + queue + Inngest functions ── Postgres
            │  ▲
   /run …   │  │  reports every 5s (discovery = heartbeat)
            ▼  │
        apps/agent ── Bun daemon: simctl / adb / maestro / screenshots
            │
            ▼
   simulators · emulators · physical devices
```
